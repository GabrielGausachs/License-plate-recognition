...
modelo_cargado = load_model("../utils/saved_models/model_cnn.h5")
...
interpreter = tf.lite.Interpreter(
    model_path=os.path.join(os.path.dirname(__file__), "model_cifar.tflite")
)
...
def identify_character(img, print_img=False):
    ...
    # Perform text extraction with the loaded model tflite
    input_details = interpreter.get_input_details()
    output_details = interpreter.get_output_details()

    interpreter.set_tensor(input_details[0]["index"], invert.astype(np.float32))
    interpreter.invoke()

    data = interpreter.get_tensor(output_details[0]["index"])
...
if __name__ == "__main__":
    ...
    # Find plate
    identify_character(image, print_img=True)
